...
export class Utils {
  ...
  /**
   * 检查邀请码在数据库中是否唯一，如不唯一则重新生成
   * @param repository 实体仓库
   * @param length 邀请码长度，默认6位
   * @returns 唯一的邀请码
   */
  async generateUniqueInviteCode(repository, length = 6): Promise<string> {
    const code = this.generateInviteCode(length);
    // 检查数据库中是否已存在该邀请码
    const exist = await repository.findOneBy({ inviteCode: code });
    if (exist) {
      // 如果已存在，递归重新生成
      return this.generateUniqueInviteCode(repository, length);
    }
    return code;
  }
}
